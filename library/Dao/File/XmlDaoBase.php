  ...
  public function interpretFileContent($content) {
    ...
    foreach ($tags as $key => $value) {
      if ($key == $this->resourceName . 'Entry') {
        ...
        for ($i = 0; $i < count($items); $i+=2) {
          if ( ! isset($items[$i]) || ! isset($items[$i + 1])) {
            file_put_contents('/tmp/TTT', $content);
            throw new XmlDaoException('The XML file seems to be incorrect for resource \'' . $this->resourceName . '\'.');
          }
          $offset = $items[$i] + 1;
      ...
      elseif ($key == 'errors') {
        ...
        for ($i = 0; $i < count($items); $i+=2) {
          if ( ! isset($items[$i]) || ! isset($items[$i + 1])) {
            throw new XmlDaoException('The XML file seems to be incorrect for resource \'' . $this->resourceName . '\'. (The errors section is not formatted correctly)');
          }
          $offset = $items[$i] + 1;
          $len = $items[$i + 1] - $offset;
          $fields = array_slice($values, $offset, $len);
      ...
      elseif ($key == 'debug') {
        ...
        for ($i = 0; $i < count($items); $i+=2) {
          if ( ! isset($items[$i]) || ! isset($items[$i + 1])) {
            throw new XmlDaoException('The XML file seems to be incorrect for resource \'' . $this->resourceName . '\'. (The debug section is not formatted correctly)');
          }
          $offset = $items[$i] + 1;
          $len = $items[$i + 1] - $offset;
          $fields = array_slice($values, $offset, $len);
